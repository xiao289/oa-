...
public class AnnounceExcelUtil {
    //1.创建一个Excel对象
    ...
    public static void createother(List list){
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            c1.setCellValue(announce.getTitle());
            c2.setCellValue(announce.getUser().getName());
            c3.setCellValue(sf.format(announce.getStarttime()));
            c4.setCellValue(sf.format(announce.getEndtime()));
            c5.setCellValue(sf.format(announce.getSuntime()));
            c6.setCellValue(announce.getContent());
