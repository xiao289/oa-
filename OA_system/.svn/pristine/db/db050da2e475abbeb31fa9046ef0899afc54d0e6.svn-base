...
public class LogExcelUtil {
    ...
    public static void createother(List list){
        for (int i = 0; i < list.size(); i++) {
            ...
            c1.setCellValue(log.getId());
            c2.setCellValue(log.getUsername());
            c3.setCellValue(log.getRolename());
            c4.setCellValue(log.getEvent());

                c5.setCellValue(log.getRemark());
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-ss hh:mm:ss");
            String format = dateFormat.format(log.getUpdatetime());
            c6.setCellValue(format);
